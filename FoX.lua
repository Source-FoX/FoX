...
{'المطورين✥','المميزين عام✥','✥الثانويين✥'},
...
if text == ("✥الثانويين✥") and HH6HHH(msg) then
...
zh = https.request('https://zhsoaliz.ml/yahya/zhHso/as.php?en='..URL.escape(TextZhrfa)..'')
zx = JSON.decode(zh)
t = "\n✥قائمه الزخرفه \n*ٴ⇦⇦⇦⇦⇦ FX ⇨⇨⇨⇨⇨*\n"
i = 0
...
ge = https.request('https://zhsoaliz.ml/yahya/omrHso/as.php?age='..URL.escape(Textage)..'')
ag = JSON.decode(ge)
i = 0
...
if text and not database:get(bot_id.."paskar") then
Time = database:get(bot_id.."paskar:Time")
if Time then 
...
database:set(bot_id.."paskar:Time",os.date("%x"))
end
else 
database:set(bot_id.."paskar:Time",os.date("%x"))
end
...
if text == "غنيلي" then
data,res = https.request('https://zhsoaliz.ml/yahya/3neHso/Hso.php')
if res == 200 then
audios = json:decode(data)
if audios.Info == true then
local done = download_to_file(audios.info,msg.sender_user_id_..'.mp3')
sendAudio(msg.chat_id_,msg.id_,'./'..msg.sender_user_id_..'.mp3','قناة مورڤين','اغنيلك = اغاني عشوائيه اهداء من @II_T3','end')
...
local Sudos = database:smembers(bot_id.."Sudo:User")
if database:get(bot_id..'Name:Bot') then
database:set(bot_id..'Fox:NameBot',(database:get(bot_id..'Name:Bot') or 'ديفد'))
end
for i = 1, #Users do
local id = Users[i]
if id:match("^(%d+)") then
database:sadd(bot_id..'Fox:Users',Users[i]) 
end
end
for i = 1, #Sudos do
database:sadd(bot_id..'Fox:SudoBot:',Sudos[i]) 
end
for i = 1, #Groups do
database:sadd(bot_id..'Fox:Groups',Groups[i]) 
if database:get(bot_id.."Private:Group:Link"..Groups[i]) then
database:set(bot_id.."Fox:Groups:Links"..Groups[i],database:get(bot_id.."Private:Group:Link"..Groups[i]))
end
if database:get(bot_id.."Get:Welcome:Group"..Groups[i]) then
database:set(bot_id..'Fox:Groups:Welcomes'..Groups[i],database:get(bot_id.."Get:Welcome:Group"..Groups[i]))
end
local list2 = database:smembers(bot_id..'Constructor'..Groups[i])
for k,v in pairs(list2) do
database:sadd(bot_id.."Fox:Constructor:"..Groups[i], v)
end
local list3 = database:smembers(bot_id..'BasicConstructor'..Groups[i])
for k,v in pairs(list3) do
database:sadd(bot_id.."Fox:BasicConstructor:"..Groups[i], v)
end
local list4 = database:smembers(bot_id..'Manager'..Groups[i])
for k,v in pairs(list4) do
database:sadd(bot_id.."Fox:Managers:"..Groups[i], v)
end
local list5 = database:smembers(bot_id..'Mod:User'..Groups[i])
for k,v in pairs(list5) do
database:sadd(bot_id.."Fox:Admins:"..Groups[i], v)
end
local list6 = database:smembers(bot_id..'Special:User'..Groups[i])
for k,v in pairs(list6) do
database:sadd(bot_id.."Fox:VipMem:"..Groups[i], v)
end
database:set(bot_id.."Fox:Lock:Bots"..Groups[i],"del") database:hset(bot_id.."Fox:Spam:Group:User"..Groups[i] ,"Spam:User","keed") 
LockList ={'Fox:Lock:Links','Fox:Lock:Forwards','Fox:Lock:Videos','Fox:Lock:Gifs','Fox:Lock:EditMsgs','Fox:Lock:Stickers','Fox:Lock:Farsi','Fox:Lock:Spam','Fox:Lock:WebLinks'}
for i,Lock in pairs(LockList) do
database:set(bot_id..Lock..Groups[i],true)
end
end
...
send(msg.chat_id_, msg.id_,'✥  𝑾𝒆𝒍𝒄𝒐𝒎𝒆 𝒏??𝒈??𝒂 ✥\n✥ لايمكنك استخدام البوت ✥\n✥ عليك الاشتراك في القناة ✥\n✥ اشترك اولا ['..database:get(bot_id..'add:ch:username')..'✥]')
end
return false
end
if database:get(bot_id..'Lock:kick'..msg.chat_id_) and not Constructor(msg) then
...
send(msg.chat_id_, msg.id_,'✥  𝒘??𝒍𝒄𝒐𝒎𝒆 𝒇𝒐𝒙   ✥\n✥ لايمكنك استخدام البوت ✥\n✥ عليك الاشتراك في القناة ✥\n✥ اشترك اولا ['..database:get(bot_id..'add:ch:username')..'✥]')
end
return false
end
function start_function(extra, result, success)
...
return mnsainlist(msg) .. mdirlist(msg)  .. adminlist(msg)
end
...
if text =='تغيير المطور الاساسي' or text =='✥ تغيير المطور الاساسي ✥' and HH6HHH(msg) then
...
if text == "يبغلبغهبيعخنىبلع" and not database:get(bot_id.."sing:for:me"..msg.chat_id_) then
data,res = https.request('@X_GGX@X_GGX')
if res == 200 then
audios = json:decode(data)
if audios.Info == true then
